                  ...
                  unsigned int nb_samples, unsigned int bits_per_sample, unsigned int size, int64_t pts )
{
    ...
    /**
     * À faire :
     * - Vérifier les tailles (doc de VLC) -> Fait ?
     * - Extraire les features et les récupérer
     * - Rajouter des delete
     * - Tester tout ça...
     */
    ...
    while(remaining < size)
    {
        ...
        if(sp->mBufferSize == AUDIO_CHUNK_SIZE)
        {
            cout << size << endl;
            // Extracting features
            sp->mSpecExtr.extract(sp->mBuffer, AUDIO_CHUNK_SIZE);
            sp->mSpecExtr.normalize(35000);

            // Dumping
            sp->dumpStreamToFile16x2(sp->mBuffer, sp->mSpecExtr.spectrum(), AUDIO_CHUNK_SIZE);
